...
function fbComment()
{
    $fbComment = \App\Plugin::where('act', 'fb-comment')->where('status', 1)->first();
...
{
    $under_ref = \App\User::where('ref_by', $id)->get();
    $print = array();
    $i = 2;
    foreach ($under_ref as $data) {
        $cc = \App\User::where('ref_by', $data->id)->count();
...
function showUserLevel($id, $level)
{
    ...
    foreach ($nxt as $key => $uu) {
        ...
        echo ' <td data-label="Username">'.$data->fullname.'</td>';
